...
- That branch runs after every server start, so a release that ships without
  its row would reach every user on every launch. With a server running it
  therefore only logs to the output channel; the modal is kept for when no
  server started, or when the user invoked the command themselves. A unit test
  pins the map to `client/package.json`'s version so the release PR's CI goes
  red instead.
