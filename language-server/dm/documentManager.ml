...
let diags_version st = Document.diags_version st.document
...
let init init_vs ~opts uri ~text =
  ...
  let state = { uri; opts; init_vs; document; document_state = Parsing; feedback_pipe; pending_feedback = []; pending_interpretation = None; checking_state } in
...
let reset { uri; opts; init_vs; document; checking_state; feedback_pipe } =
  ...
  let state = { uri; opts; init_vs; document; checking_state; document_state = Parsing; feedback_pipe; pending_feedback = []; pending_interpretation = None } in
  let parsebegin_event = Sel.now ~priority:PriorityManager.launch_parsing ParseBegin in
  state, [parsebegin_event;feedback_event]
