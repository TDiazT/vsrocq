...
describe("advise", () => {
    ...
    it("reports the server that did start when the map does not cover the extension", () => {
        // The flow runs after every server start. With a server up, a missing
        // row is a hole in our table, not a broken setup, and the presenter
        // needs to know which of the two it is looking at.
        const advice = advise({
            extensionVersion: "2.1.4",
            serverVersion: "2.4.3+1",
            configuredPath: null,
            opam: opamFacts(),
        });
...
describe("versionRequirements", () => {
    it("has a row for the extension version in package.json", () => {
        // The map is edited by hand in each release commit. A release that
        // forgets it leaves the extension unable to name the server it needs,
        // and the only cheap moment to catch that is the release PR's CI.
        // Compiled tests run from out/test/unit, three levels below client/.
        const packageJson = JSON.parse(
            readFileSync(join(__dirname, "../../../package.json"), "utf8"),
        ) as { version: string };

        expect(versionRequirements[packageJson.version]).toBeDefined();
    });
});
