...
export async function present(
    ...
    switch (advice.kind) {
        ...
        case "unknownExtensionVersion":
            if (advice.serverVersion !== null && !announceOk) {
                // A server started and answered. Whether it is new enough
                // cannot be judged without a row, but the only thing known to
                // be wrong is our table, and this runs on every start: a
                // modal here would reach every user on every launch until a
                // release fills the row in.
                Client.writeToVsrocqChannel(
                    "[Versioning] No known server requirement for VsRocq " +
                        advice.extension +
                        "; server " +
                        advice.serverVersion +
            ...
            window
                .showWarningMessage(
                    `No known language server requirement for VsRocq ${advice.extension}.`,
                    {
                        modal: true,
                        detail:
                            advice.serverVersion === null
                                ? "This extension build is not in the compatibility table, so the required server version cannot be determined. Install the language server matching this extension's version."
                                : `This extension build is not in the compatibility table, so whether language server ${advice.serverVersion} is new enough cannot be determined.`,
                    },
                    DOCS_BUTTON,
                )
